...
const CartItemRow = ({ item }) => {
  ...
  const remove = window.cartStore.getState().remove;
  const setQty = window.cartStore.getState().setQty;
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12 }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          <h4 style={{
            ...
            fontSize: 22, lineHeight: 1.1, margin: 0, color: '#1f3028',
          }}>{item.name}</h4>
          <div style={{ marginTop: 8, display: 'flex', flexWrap: 'wrap', gap: 6, alignItems: 'center' }}>
            <span style={{
              fontFamily: 'Montserrat, sans-serif', fontSize: 11, fontWeight: 500,
              padding: '4px 9px', borderRadius: 999,
              background: '#8faf8a55', color: '#1f3028', border: '1px solid #8faf8a99',
            }}>{item.weight} · {item.badge}</span>
            <span style={{
              fontFamily: 'Montserrat, sans-serif', fontSize: 11, color: '#533b22',
            }}>{formatPEN(item.unitCents)} c/u</span>
          </div>
          <div style={{
            fontFamily: 'Montserrat, sans-serif', fontSize: 11, color: '#533b22',
            marginTop: 6,
        ...
        <button onClick={() => remove(item.id)} style={{
          fontFamily: 'Montserrat, sans-serif', fontSize: 11, fontWeight: 500,
          color: '#c96e4b', background: 'transparent', border: 'none', cursor: 'pointer',
          padding: 4, letterSpacing: '0.04em',
        }}>Eliminar</button>
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
        <div style={{
          ...
          background: '#f2e0cc', border: '1px solid #1f302833', borderRadius: 999,
        }}>
          <button onClick={() => dec(item.id)} aria-label="Restar" style={{
            width: 36, height: 36, border: 'none', background: 'transparent',
            cursor: 'pointer', fontSize: 18, color: '#1f3028',
          }}>−</button>
          <input
            type="number"
            value={item.qty}
            ...
            style={{
              width: 36, textAlign: 'center', border: 'none', background: 'transparent',
              fontFamily: 'Montserrat, sans-serif', fontSize: 14, fontWeight: 600, color: '#1f3028',
              outline: 'none',
              MozAppearance: 'textfield',
            }}
          />
          <button onClick={() => inc(item.id)} aria-label="Sumar" disabled={item.qty >= item.maxQty} style={{
            width: 36, height: 36, border: 'none', background: 'transparent',
            cursor: item.qty >= item.maxQty ? 'not-allowed' : 'pointer',
            fontSize: 18, color: item.qty >= item.maxQty ? '#1f302855' : '#1f3028',
          }}>+</button>
        </div>
        <div style={{
          fontFamily: 'Cormorant Garamond, serif', fontWeight: 700,
          fontSize: 24, color: '#1f3028',
        }}>{formatPEN(lineTotal)}</div>
      </div>
      <div style={{
...
const EmptyCart = () => (
  <div style={{
    ...
    justifyContent: 'center', padding: '60px 20px', textAlign: 'center', gap: 18,
  }}>
    <div style={{
      width: 96, height: 96, borderRadius: '50%', background: '#f2e0cc',
    ...
    <h3 style={{
      fontFamily: 'Cormorant Garamond, serif', fontWeight: 600,
      fontSize: 30, color: '#f2e0cc', margin: 0, lineHeight: 1.1,
    }}>Tu canasta está vacía.</h3>
    <p style={{
    ...
    <a href="#cafe" onClick={() => window.cartStore.getState().close()} style={{
      fontFamily: 'Montserrat, sans-serif', fontSize: 13, fontWeight: 600,
      letterSpacing: '0.06em', textTransform: 'uppercase',
      color: '#1f3028', background: '#c96e4b',
      padding: '14px 22px', borderRadius: 999, textDecoration: 'none',
      marginTop: 8,
    }}>Explorar la cosecha</a>
  </div>
);

const CartDrawer = () => {
  const isOpen = window.useCart((s) => s.isOpen);
  ...
  const cycle = window.useCart((s) => s.cycle);
  const close = window.cartStore.getState().close;
  const clear = window.cartStore.getState().clear;
  const openCheckout = window.cartStore.getState().openCheckout;
  ...
  return (
    <React.Fragment>
      ...
      <aside aria-hidden={!isOpen} style={{
        ...
        boxShadow: '-30px 0 80px -30px #000000aa',
        borderLeft: '1px solid #c96e4b33',
      }}>
        {/* header */}
        <div style={{
          padding: '24px 28px 18px', borderBottom: '1px solid #f2e0cc18',
          ...
          <div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <span style={{
                fontFamily: 'Montserrat, sans-serif', fontSize: 12, color: '#c4b297',
              }}>{totalQuantity} {totalQuantity === 1 ? 'artículo' : 'artículos'}</span>
            </div>
          </div>
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <button onClick={close} aria-label="Cerrar" style={{
              ...
              background: '#f2e0cc11', border: '1px solid #f2e0cc22',
              color: '#f2e0cc', cursor: 'pointer', fontSize: 14,
            }}>✕</button>
          </div>
        </div>
        ...
        {items.length > 0 && (
          <div style={{
            margin: '18px 28px 0', padding: '12px 16px', borderRadius: 12,
        ...
        <div style={{
          ...
          padding: items.length ? '18px 28px' : 0,
        ...
        {items.length > 0 && (
          <div style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
              <span style={{
                fontFamily: 'Montserrat, sans-serif', fontSize: 13, color: '#c4b297',
              }}>Subtotal (sin delivery)</span>
              <span style={{
                fontFamily: 'Cormorant Garamond, serif', fontWeight: 700, fontSize: 32, color: '#f2e0cc',
              }}>{formatPEN(subtotalCents)}</span>
            </div>
            <div style={{
              fontFamily: 'JetBrains Mono, monospace', fontSize: 10, letterSpacing: '0.18em',
              color: '#8faf8a',
            ...
            <button onClick={openCheckout} style={{
              ...
              padding: '20px 24px', borderRadius: 999, border: 'none', cursor: 'pointer',
            ...
            <div style={{
              fontFamily: 'Montserrat, sans-serif', fontSize: 11, color: '#c4b29799',
              textAlign: 'center', lineHeight: 1.6,
            }}>
              Envío gratis Lima desde S/ 100 · Provincia desde S/ 150
              <br />
